/* tslint:disable */
/* eslint-disable */
/**
 * OffTheRecord Rest Service API - Devo
 * A service to handle your traffic tickets
 *
 * The version of the OpenAPI document: 1.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface SignImageUrlRequest
 */
export interface SignImageUrlRequest {
    /**
     * 
     * @type {number}
     * @memberof SignImageUrlRequest
     */
    minutes?: number;
    /**
     * 
     * @type {string}
     * @memberof SignImageUrlRequest
     */
    unsignedImageUrl?: string;
}

export function SignImageUrlRequestFromJSON(json: any): SignImageUrlRequest {
    return SignImageUrlRequestFromJSONTyped(json, false);
}

export function SignImageUrlRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): SignImageUrlRequest {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'minutes': !exists(json, 'minutes') ? undefined : json['minutes'],
        'unsignedImageUrl': !exists(json, 'unsignedImageUrl') ? undefined : json['unsignedImageUrl'],
    };
}

export function SignImageUrlRequestToJSON(value?: SignImageUrlRequest | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'minutes': value.minutes,
        'unsignedImageUrl': value.unsignedImageUrl,
    };
}


